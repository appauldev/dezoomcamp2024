...
resource "google_service_account" "wk3_hw_mage_sa" {
  account_id   = var.wk3_hw_mage_sa_id
  display_name = var.wk3_hw_mage_sa_name
}

# the SA is given an admin access to the specific bucket we will create below
resource "google_storage_bucket_iam_member" "member" {
  bucket = google_storage_bucket.wk3_hw_bucket.name
  role   = "roles/storage.admin"
  member = "serviceAccount:${google_service_account.wk3_hw_mage_sa.email}"

}

resource "google_storage_bucket" "wk3_hw_bucket" {
  name          = "wk3_hw_bucket"
